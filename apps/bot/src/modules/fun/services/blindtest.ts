...
import { getOrCreateConfiguredPlayer } from "../../music/services/playerUtils";
import {
  BLINDTEST_CATEGORIES,
  type BlindtestCategory,
  type BlindtestTrack,
} from "../data/blindtestCategories";
import { artistAnswers, matchesAny, titleAnswers } from "./blindtestAnswers";
...
export function findBlindtestCategory(id: string): BlindtestCategory | undefined {
  return BLINDTEST_CATEGORIES.find((category) => category.id === id);
}
...
function controlRow(guildId: string) {
  return funRow(
    funButton(`fun:blindtest-skip:${guildId}`, "Passer la manche"),
    funButton(`fun:blindtest-stop:${guildId}`, "Arrêter", ButtonStyle.Danger),
  );
}

function renderIntro(session: Session): FunPayload {
  return funPayload(
    [
      `### Blindtest : ${session.category.name}`,
    ...
    [funRow(funButton(`fun:blindtest-stop:${session.guildId}`, "Arrêter", ButtonStyle.Danger))],
  );
}

function renderRound(session: Session, round: Round): FunPayload {
  ...
  return funPayload(lines, over ? [] : [controlRow(session.guildId)]);
}

function renderFinal(session: Session, reason: FinishReason, stoppedBy?: string): FunPayload {
  ...
  const details: Record<FinishReason, string> = {
    completed: `${plural(session.played, "manche")} jouée(s) en catégorie ${session.category.name}.`,
  ...
  return funPayload([
    `### ${titles[reason]} : ${session.category.name}`,
    details[reason],
    "",
    "**Classement final**",
...
export async function startBlindtest(
  ...
  const { member, channel } = options;
  const guildId = member.guild.id;

  const category = findBlindtestCategory(options.categoryId);
  if (!category) throw new GauliaError("Cette catégorie n'existe pas.");
