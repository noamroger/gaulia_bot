...
import { formatDuration, formatNumber, gold, withExploreButtonSafe } from "./shared";
...
export function exploreView(outcome: ExploreOutcome): V2MessagePayload {
  ...
  return withExploreButtonSafe(
    toV2Payload(false, buildContainer(color, lines)),
    outcome.character.userId,
  );
}
...
export function dungeonStatusView(
  ...
  const lines = [
    ...
    status.cooldownMs > 0
      ...
      : "Le passage est ouvert : `/aventure donjon lancer:true` pour l'affronter.",
